...
func onTrack() -> bool :
	...
	if textureAtlas == Vector2i(11,0): #curvado, de izq a arr
	...
	elif textureAtlas == Vector2i(6,0) or textureAtlas == Vector2i(8,0): # asumimos que es el horizontal recto O horizontal recto con boton
		...
		if textureAtlas == Vector2i(8,0) and not train.isCargo: #si tiene boton y no es cargo, gira todas.
